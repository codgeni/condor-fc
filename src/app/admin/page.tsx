...
export default function AdminPanel() {
  ...
  const savePlayerEdit = async (e: React.FormEvent) => {
    ...
    if (editingPlayer.detail_img || editingPlayer.detailImg) {
      updatePayload.detail_img = editingPlayer.detail_img || editingPlayer.detailImg;
    }
    const { error } = await supabase.from('players').update(updatePayload).eq('id', editingPlayer.id);
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {editingPlayer ? (
              <form onSubmit={savePlayerEdit} style={{ background: 'white', padding: '2rem', borderRadius: '12px', border: '1px solid #eee', marginBottom: '2rem' }}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '5px' }}>Photo de célébration / profil intérieur (Page Détail Joueur)</label>
                  <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    {(editingPlayer.detail_img || editingPlayer.detailImg) && (
                      <img src={editingPlayer.detail_img || editingPlayer.detailImg} style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover', border: '1px solid #ddd' }} alt="Aperçu célébration" />
                    )}
                    <div style={{
                      flex: 1,
                      border: '2px dashed #ccc',
                      ...
                      />
                      <span style={{ fontSize: '0.85rem', color: '#555', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}>
                        <Upload size={14} /> {(editingPlayer.detail_img || editingPlayer.detailImg) ? "Changer la photo de célébration" : "Importer photo de célébration"}
                      </span>
                    </div>
                  </div>
                </div>
