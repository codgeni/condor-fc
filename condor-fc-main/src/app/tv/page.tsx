...
export default function CondorTV() {
  ...
  return (
    <div style={{ flex: 1, marginTop: '80px', background: 'var(--clr-black)', color: 'white', overflowX: 'hidden' }}>
      ...
      {/* 4. The Vault / Classic Matches */}
      <section className="section-padding">
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', background: 'linear-gradient(45deg, #1a0000, #330000)', padding: '4rem', borderRadius: '16px', alignItems: 'center' }}>
            <div>
              <span style={{ color: 'var(--clr-primary)', fontWeight: 'bold', letterSpacing: '2px' }}>LE COFFRE-FORT</span>
              <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '3.5rem', margin: '15px 0' }}>Matchs Classiques</h2>
              <p style={{ fontSize: '1.2rem', color: '#ccc', marginBottom: '2rem' }}>Revivez les plus grands retours, les finales historiques et les performances légendaires qui ont défini le Condor FC.</p>
              <button className="btn btn-primary">Ouvrir Le Coffre-Fort</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
              <VideoCard img="/player_action_2_1780681894021.png" time="90:00" title="Match Complet : Finale LDC 2022" />
              <VideoCard img="/player_action_1_1780681882713.png" time="90:00" title="Match Complet : La Victoire 5-0 (2018)" />
            </div>
          </div>
        </div>
      </section>
